...
$backend = if ($env:WHISPR_GATEWAY_BACKEND) { $env:WHISPR_GATEWAY_BACKEND.ToLower() } else { "cuda" }
if ($backend -notin @("cuda", "cpu")) {
  throw "Unsupported backend '$backend' (expected cuda or cpu)"
}

py -m venv $venv
& "$venv\Scripts\python.exe" -m pip install --upgrade pip
$backendRequirements = if ($backend -eq "cuda") { "$gw\requirements-local.txt" } else { "$gw\requirements-faster.txt" }
& "$venv\Scripts\pip.exe" install -r "$gw\requirements.txt" -r $backendRequirements pyinstaller
...
if ($backend -eq "cuda") { $collect += @("--collect-all", "nvidia") }
